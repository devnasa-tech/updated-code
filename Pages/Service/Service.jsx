...
const companyData = [
  ...
  },
  {
    ...
    "notes": "International compliance codes and national regulations are strictly upheld throughout the entire process of manufacturing our RMG products. Since our clients' satisfaction is a priority, we leave no stone unturned regarding quality and ensure that our market presence is eco-friendly.",
    "image": "https://i.ibb.co/Q33sKNGy/Textile-Production-Line-Sewing-Machines-in-Action-Aaryan-Sourcing.jpg"
  },
  {
    id: 7,
  ...
  },
  {
    ...
    notes: "Since the apparel industry is highly sensitive, various chemical tests are conducted on all items before approval according to the customer’s requirements.",
...
const companyInfo = companyData.reduce((acc, item) => {
  acc[item.title] = item;
  return acc;
}, {});
...
const Service = () => {
  ...
  const currentContent = companyInfo[activeSection];
  ...
  return (
    <div className="bg-gray-50">
      ...
      <section className="bg-white py-16 sm:py-20 border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg text-gray-600 leading-relaxed mb-10">
            Aaryan Sourcing supplies apparel from concept to completion and is equipped to handle your garment manufacturing and delivery needs. Our dedicated fabric sourcing, customized garment production, and rigorous quality testing ensure that specific products meet industry standards. We work with you to create fabulous garments, focusing on product patterns, fabric texture, and production time.
          </p>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            <Link href="/workprocess" className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3 font-semibold text-white bg-yellow-500 rounded-lg shadow-md hover:bg-yellow-600 transition-colors duration-300">
              <span>Learn more about our sourcing process</span>
              <FaArrowRight />
            </Link>
            <Link href="/Sustainability" className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3 font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors duration-300">
              <span>Explore our sustainability practices</span>
              <FaArrowRight />
            </Link>
            <Link href="/profile" className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3 font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto text-gray-800">
        <div className="mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 flex flex-col md:flex-row gap-8">
            ...
            <div className="bg-white border border-gray-200 p-6 rounded-lg shadow-sm sticky top-6">
              ...
              <nav className="space-y-1">
                {companyData.map((item) => (
                  <button
                    key={item.id}
                    aria-current={activeSection === item.title ? "page" : undefined}
                    className={`flex justify-between items-center w-full text-left py-3 px-4 cursor-pointer text-base transition-all duration-200 rounded-lg ${activeSection === item.title
          ...
          <main className="w-full md:w-3/4">
            <div className="bg-white rounded-lg shadow-sm p-6 md:p-8 border border-gray-100">
              ...
              <div className="mb-8 rounded-lg overflow-hidden shadow-md relative w-full h-64 md:h-80">
                <img
              ...
              <div className="prose prose-lg max-w-none space-y-8">
                {Object.entries(currentContent).map(([key, value]) => {
                  if (["id", "title", "subtitle", "description", "image", "notes"].includes(key))
                    return null;
                ...
                {currentContent.notes && (
                  <div className="space-y-5">
                    <div className="bg-yellow-50 p-5 rounded-lg border-l-4 border-yellow-400">
                      <p className="text-gray-700 italic m-0">{currentContent.notes}</p>
                    </div>
                  </div>
                )}
              </div>
