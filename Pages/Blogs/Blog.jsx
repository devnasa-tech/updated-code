...
const Blog = () => {
  ...
  if (error) {
    return <div className="text-center py-20 text-lg text-red-500">Error: {error}</div>;
  }

  const featuredBlog = blogs[0];
  const otherBlogs = blogs.slice(1);
  const featuredDate = featuredBlog ? formatDate(featuredBlog.createdAt) : null;
        ...
        <div className="bg-white grid grid-cols-1 lg:grid-cols-2 px-4 sm:px-10 md:px-16 lg:px-32 my-10">
          <div className="relative max-w-6xl mx-auto h-[250px] lg:h-auto w-full bg-gray-200">
            ...
            <div className="absolute top-0 left-0 bg-yellow-400 text-gray-800 text-center flex flex-col px-4 py-1">
              ...
              <span className="text-base tracking-widest">{featuredDate.month}</span>
            </div>
          </div>
          <div className="relative w-full px-8 pt-8 bg-[#EFEFEF]">
            <div className="pb-16">
              ...
              {featuredBlog.shortDescription && (
                <div className="prose prose-sm max-w-none text-gray-600 leading-relaxed mb-4 line-clamp-4">
                  <RichTextRenderer content={featuredBlog.shortDescription} />
                </div>
              )}
              <div className="flex items-center space-x-6 text-gray-500 text-sm mb-4">
                <div className="flex items-center space-x-2"><FaUser /><span>Admin</span></div>
                <div className="flex items-center space-x-2"><FaRegComment /><span>0</span></div>
              </div>
              {featuredBlog.tags && (
                <div className="flex items-center flex-wrap gap-2 mb-10">
                  <FaTag className="text-gray-500" />
                  {featuredBlog.tags.split(',').map(tag => tag.trim()).map((tag, index) => (
            ...
            <div className="absolute bottom-0 left-0 text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-semibold text-[#E3E3E3] select-none pl-6 pointer-events-none">
              {featuredBlog.category.toUpperCase()}
            </div>
            <Link href={`/blog/${featuredBlog._id}`} className="absolute bottom-0 right-0 bg-yellow-400 p-6 hover:bg-yellow-500 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {otherBlogs.map(blog => {
            const postForCard = {
              ...
              description: blog.shortDescription || blog.content,
              tags: blog.tags || '',
            ...
            return (
              <BlogPostCard key={blog._id} post={postForCard} />
            );
          })}
        </div>
      </div>
